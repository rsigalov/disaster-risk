...
svi_data = CSV.read(string("data/raw_data_new/svi_params_", index_to_append, ".csv"); datarow = 2, delim = ",")
...
print("\n--- First Pass ---\n")
@time tmp = pmap(estimate_parameters_short, spot_arr[1:2], r_arr[1:2], F_arr[1:2], T_arr[1:2],
    sigma_NTM_arr[1:2],min_K_arr[1:2], max_K_arr[1:2], svi_arr[1:2])

print("\n--- Second Pass ---\n")
@time ests = pmap(estimate_parameters_clamp, spot_arr, r_arr,
...
df_out = DataFrame(secid = svi_data.secid,
              ...
              T = T_arr,
              V_clamp = map(x -> x[1], ests),
              IV_clamp = map(x -> x[2], ests))

CSV.write(string("data/output_new/var_ests_", index_to_append, ".csv"), df_out)
