...
################################################################
var_to_calculate = :rn_prob_40mon
obs_filter = 15
share_months = 0.8
...
df = df[:, [:secid, :date, var_to_calculate]]
df[:date_adj] = Dates.lastdayofmonth.(df[:date])
...
df_filter_secid = df_filter_secid[:, [:secid, :date_adj, :D]]
...
df_pivot = unstack(df_filter_secid, :date_adj, :secid, :D)
...
df_to_output = DataFrame(
    date = date_vec[:],
    D_pc1 = Dw,
    D_mean_filter = D_mean_filter[:],
    D_mean_all = D_average_all.D)

corr_mat = cor(convert(Array{Float64,2}, df_to_output[:, [:D_pc1, :D_mean_filter, :D_mean_all]]))

CSV.write(string("estimated_data/disaster-risk-series/agg_prob_40perc_30days.csv"), df_to_output)
